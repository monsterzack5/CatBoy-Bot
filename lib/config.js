...
module.exports.run = async (message, args, command, bot) => {
   ...
   switch (args[0].toLowerCase()) {
      case 'update':
         ...
         return fileLoader.exportFile(bot, `${process.env.config_file}.json`);

      case 'updatedb':
         message.channel.send(`Updating ${process.env.db_file} using local version`);
         return fileLoader.exportFile(bot, `${process.env.db_file}.json`);
      ...
      case 'update4chan':
         ...
         return chan.update(bot);

      case 'dump4chan':
         return message.channel.send({
            files: [`${process.env.db_file}.json`],
         });
