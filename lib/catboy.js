...
fs.watch(`./${process.env.db_file}.json`, () => {
   if (process.uptime() > 30) {
      console.log('DB Has been modifed and has been updated!');
      db = JSON.parse(fs.readFileSync(`./${process.env.db_file}.json`));
   }
});
...
function bingCat() {
   return new Promise(async (resolve) => {
      ...
      console.log(`random cat is ${random}`);
      const reply = await embedBuilder(null, db.images[random].url);
      return resolve(reply);
   });
}
...
module.exports.run = async (message) => {
   ...
   const randomSearch = Math.floor(Math.random() * 45);
   let reply;
