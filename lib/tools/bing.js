...
const insert = db.prepare('INSERT OR REPLACE INTO bingcats (id, url) VALUES(?, ?)');

const insertAll = db.transaction((images) => {
   for (const image of images) {
      insert.run(image.id, image.url);
   }
});
...
async function update() {
   ...
   // after we insert our new data, we should removed the filtered IDs
}
